...
build:
	...
	go build -o greenera -ldflags="\
    	-X 'github.com/ksandr84on/nera/versioning.Version=$(LATEST_VERSION)' \
		-X 'github.com/ksandr84on/nera/versioning.Commit=$(COMMIT_HASH)'\
		-X 'github.com/ksandr84on/nera/versioning.Branch=$(BRANCH)'\
		-X 'github.com/ksandr84on/nera/versioning.BuildTime=$(TIME)'" \
	main.go
...
test-e2e:
	...
	go build -race -o artifacts/greenera .
	env EDGE_BINARY=${PWD}/artifacts/greenera go test -v -timeout=30m ./e2e/...
